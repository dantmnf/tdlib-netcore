...
EXPORT void td_bridge_client_send(Client *client, std::uint64_t id, td_api::Function *func) {
    Client::Request req {id, object_ptr<Function>(func)};
    client->send(std::move(req));
}

EXPORT Object *td_bridge_client_receive(Client *client, double timeout, std::int64_t *id) {
    auto result = client->receive(timeout);
    *id = result.id;
    return result.object.release();
}

EXPORT Object * td_bridge_client_execute(Client *client, std::uint64_t inid, td_api::Function *func, std::int64_t *outid) {
